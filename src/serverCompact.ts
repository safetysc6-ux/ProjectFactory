...
/**
 * Small MCP surface for coding sessions.
 * The goal is to keep tool schemas and tool results small so the model spends
...
export function buildCompactMcpServer(){
  ...
  s.tool("projects","List managed projects with only routing fields",{},async()=>{
    try{
      const rows=(await listProjects()).map((p:any)=>({name:p.name,status:p.status,github:p.github_repo||null,url:p.vercel_url||null}));
      return ok(rows);
  ...
  s.tool("status","Get compact project status",{project:z.string()},async({project})=>{
    try{
      ...
      return ok({name:p.name,status:p.status,github:p.github_repo||null,branch:p.github_branch||"main",url:p.vercel_url||null,drive:Boolean(p.drive_folder_id),database:Boolean(p.supabase_project_ref)});
    }catch(e){return fail(e)}
  });

  s.tool("read_file","Read one project file. Use only files relevant to the requested change.",{
    project:z.string(),path:z.string(),branch:z.string().optional()
  },async({project,path,branch})=>{
    try{
      const p=await findProject(project);if(!p?.github_repo)throw new Error("Project has no GitHub repo");
      const f=await getFile(p.github_repo,path,branch||p.github_branch||"main");
      return ok({path:f.path,sha:f.sha,content:f.content});
    }catch(e){return fail(e)}
  });

  s.tool("patch_file","Preferred edit tool. Replace only exact text fragments instead of resending the whole file.",{
    project:z.string(),
    path:z.string(),
    changes:z.array(z.object({search:z.string().min(1),replace:z.string(),replace_all:z.boolean().default(false)})).min(1).max(20),
    message:z.string().default("feat: targeted update")
  },async({project,path,changes,message})=>{
    try{
      const p=await findProject(project);if(!p?.github_repo)throw new Error("Project has no GitHub repo");
      if(p.status==="ARCHIVED")throw new Error("Project is archived");
      p.status="CODING";await saveProject(p);
      const r=await replaceFileText(p.github_repo,path,changes,message,p.github_branch||"main");
      p.status="READY";await saveProject(p);
      return ok({path:r.path,commit:r.commit,applied:r.applied});
    }catch(e){return fail(e)}
  });

  s.tool("write_file","Create or fully replace one file. Prefer patch_file for edits to existing files.",{
    project:z.string(),path:z.string(),content:z.string(),message:z.string().default("feat: write file")
  },async({project,path,content,message})=>{
    try{
      const p=await findProject(project);if(!p?.github_repo)throw new Error("Project has no GitHub repo");
      let sha:string|undefined;try{sha=(await getFile(p.github_repo,path,p.github_branch||"main")).sha}catch{}
      const r=await putFile(p.github_repo,path,content,message,p.github_branch||"main",sha);
      return ok({path:r.path,commit:r.commit});
    }catch(e){return fail(e)}
  });

  s.tool("deploy","Deploy current project branch. Returns only the useful result, not verbose deployment logs.",{
    project:z.string()
  },async({project})=>{
    try{
      const p=await findProject(project);if(!p?.github_repo)throw new Error("Project has no GitHub repo");
      ...
      const d:any=await createDeployment(p.vercel_project_id!,repo.id,p.github_branch||"main",p.name);
